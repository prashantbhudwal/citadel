...
async function main() {
  console.log(
    '📦 Extracting Short Solo Studio Songs (< 200 words) to JSON...\n',
  )
  ...
  const filtered = allSongs
    .filter((s) => {
      ...
      return count >= 50 && count < 200 // Filter for short songs
    })
    .map((s) => ({
      id: s.id,
      title: s.title,
  ...
  const outPath = path.resolve(process.cwd(), 'data/solo-studio-under-200.json')
